 ...
 * These may also be set by the application by using the setConfigParameterValue
 * methods.
 * 
 * 
 */
...
public class AnnotationPrinter extends CasConsumer_ImplBase {
	 ...
	 * In this case, the CAS index is iterated over selected annotations and
	 * printed out into an output file
	 * 
	 * @param aCAS
	...
	public void process(CAS aCAS) throws AnalysisEngineProcessException {
		...
		String docUri = null;
		Iterator it = jcas.getAnnotationIndex(SourceDocumentInformation.type)
				.iterator();
		if (it.hasNext()) {
		...
		while (annotationIter.hasNext()) {
			...
			if (aText == "") aText = annot.toString();
			aText = aText.replace('\n', ' ');
			aText = aText.replace('\r', ' ');
			// System.out.println( annot.getType().getName() + " "+aText);
			try {
				fileWriter
						.write(annot.getType().getName() + " " + aText + "\n");
				fileWriter.flush();
				if (oConsole) {
					System.out.println(annot.getType().getName() + " " + aText);
					System.out.flush();
	 ...
	 */
	public void reconfigure(UimaContext context)
			throws ResourceConfigurationException {
		// super.reconfigure();
		// extract configuration parameter settings
		String oPath = (String) context.getConfigParameterValue("outputFile");
